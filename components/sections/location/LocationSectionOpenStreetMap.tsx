...
/**
 * Location section with OpenStreetMap embed (no API key required).
 * Displays map, address, contact info, hours, and open/closed status.
 *
 * @example
 * <LocationSectionOpenStreetMap />
 *
 * @example
 * <LocationSectionOpenStreetMap title="Visit Us" bookingText="Schedule a Visit" />
 */
export function LocationSectionOpenStreetMap({
  title = "Find Us",
  ...
  const getOpenStreetMapEmbedUrl = () => {
    ...
    // Calculate bounding box (approximately 0.02 degrees padding)
    const lonMin = longitude - 0.02
    const latMin = latitude - 0.02
    const lonMax = longitude + 0.02
    const latMax = latitude + 0.02
